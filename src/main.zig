...
pub fn addVector(id: c.usearch_key_t, vector: []const f32) !void {
    ...
    if (c.usearch_add(usearchIndex.*, id, @ptrCast(vector.ptr), vector.len) != 0) {
...
pub fn searchVector(query_vector: []const f32, top_k: usize) !void {
    ...
    const results_ids: [10]c.usearch_key_t = undefined;
    const results_distances: [10]f32 = undefined;

    const count = c.usearch_search(usearchIndex.*, @ptrCast(query_vector.ptr), query_vector.len, results_ids.ptr, results_distances.ptr, top_k);
